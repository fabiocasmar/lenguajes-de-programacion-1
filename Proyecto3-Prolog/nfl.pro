...
% El primer argumento indica la división en la AFC y el
%    segundo argumento indica la división en la NFC
inter(east,west).
inter(north,east).
inter(south,north).
inter(west,south).

% divisionales(Partido)
%
...
%position(Partido)
%
% Predicado que genera los partidos contra equipos que quedaron en
% la posisión p de su misma conferencia.
...
% common_team(Games,Teams)
%
% Predicado que triunfa si en una lista Games de juegos hay equipos en
% común con una lista Teams de equipos.
...
% selectN(N,L,[X|S])
%
% Predicado que seleciona N elementos de una lista L y
%	los coloca en un conjunto S.
...
% @param [Head|Tail] lista a eliminar elementos
% @param L2 se eliminar de la primera los elementos que esten en L2.
% @param L3 lista con los elementos eliminados
subtract([], _, []).
...
% take(N,[L|List],[L|Taken])
%
% Predicado que toma los primero N elementos de una lista L y los retorna
%	 en una nueva lista.
% @param N cantidad de elementos a tomar
% @param [L|List] lista de la cual se van a tomar los elementos
% @param [L|Taken] lista de la cual se colocan los elementos tomados
...
% play([[X,Y]|Week],Teams,Lista)
%
% Predicados que verifica qué equipos juegan una semana, los elimina y retorna
% en resultado en una nueva lista.
% @param [[X,Y]|Week] semana y juegos.
% @param Teams equipos que juegan una semana
% @param Lista lista de retorno con elimina los equipos que juegan una semana.

play([],Teams,Teams):-
	!.

play([[X,Y]|Week],Teams,Lista):-
	select(X,Teams,Nueva1),
	select(Y,Nueva1,Nueva2),
	play(Week,Nueva2,Lista).

% generate([W|Cal],Teams,[B|Byes])
%
